...
void main() {
  ...
  group('getClientProjectCounts', () {
    test(
      'should return grouped project counts when remote source is successful',
      () async {
        when(() => remoteDataSrc.getClientProjectCounts()).thenAnswer(
          (_) async => {'client-1': 2},
        );

        final result = await repoImpl.getClientProjectCounts();

        expect(result.isRight(), isTrue);
        expect(result.getOrElse(() => const <String, int>{}), {
          'client-1': 2,
        });
        verify(() => remoteDataSrc.getClientProjectCounts()).called(1);
        verifyNoMoreInteractions(remoteDataSrc);
      },
    );

    test(
      'should return ServerFailure when grouped counts fail',
      () async {
        when(
          () => remoteDataSrc.getClientProjectCounts(),
        ).thenThrow(tException);
