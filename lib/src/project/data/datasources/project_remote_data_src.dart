  ...
  Future<void> addProject(Project project) async {
    try {
      ...
      if (project.image != null && project.imageIsFile) {
        final url = await _uploadProjectFeatureImage(
          filePath: project.image!,
          projectId: projectRef.id,
        );
        projectModel = projectModel.copyWith(image: url);
      }
      if (project.images.isNotEmpty) {
        final images = await _uploadProjectImages(
    ...
    try {
      ...
      if (updatedProject['images'] != null) {
        if (updatedProject
          ...
          await projectDoc.update({'images': FieldValue.arrayUnion(images)});
          updatedProject
            ..remove('images')
            ..remove('imagesModeRegistry');
        } else {
      ...
      if (updatedProject
        ...
        final image = await _uploadProjectFeatureImage(
          filePath: filePath,
          projectId: projectId,
        );
      ...
      if (updatedProject['urls'] != null) {
        ...
        await projectDoc.update({'urls': FieldValue.arrayUnion(urls)});
        updatedProject.remove('urls');
      }
      if (updatedProject['tools'] != null) {
        final tools = updatedProject['tools'] as List<String>;
        await projectDoc.update({'tools': FieldValue.arrayUnion(tools)});
        updatedProject.remove('tools');
      }
      if (updatedProject['notes'] != null) {
        final notes = updatedProject['notes'] as List<String>;
        await projectDoc.update({'notes': FieldValue.arrayUnion(notes)});
        updatedProject.remove('notes');
      }
